...
const App: React.FC = () => {
  ...
  const calculateUniqueArea = useCallback((fields: Field[], year: number) => {
    // Mapa: Numer_Rejestracyjny -> Max_PEG
    const uniqueParcels = new Map<string, number>();
    
    fields.forEach(f => {
        const entry = f.history?.find(h => h.year === year);
        if (!entry) return;
        
        // Normalizacja klucza: usuwamy spacje, kropki i zera wiodące
        const key = (f.registrationNumber || f.name || f.id).trim().toLowerCase().replace(/[\s\.\-_]/g, '');
        
        // Sumujemy TYLKO eligibleArea (Hektar kwalifikujący się ogółem)
        const currentPeg = entry.eligibleArea || 0;
        
        // Jeśli działka już istnieje, bierzemy wyższą wartość (zabezpieczenie przed dublowaniem rekordów)
        if (!uniqueParcels.has(key) || uniqueParcels.get(key)! < currentPeg) {
            uniqueParcels.set(key, currentPeg);
        }
    });

    let total = 0;
    uniqueParcels.forEach(val => total += val);
  ...
  const renderContent = () => {
    ...
    switch (currentView) {
      ...
      case 'ADMIN': return <AdminPanel templates={csvTemplates} onSaveTemplate={(t) => setCsvTemplates(p => [...p.filter(x => x.id !== t.id), t])} onDeleteTemplate={(id) => setCsvTemplates(p => p.filter(x => x.id !== id))} selectedGlobalYear={selectedYear} />;
