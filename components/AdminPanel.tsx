...
const AdminPanel: React.FC<AdminPanelProps> = ({ templates, onSaveTemplate, onDeleteTemplate, selectedGlobalYear, clients }) => {
    ...
    const [selectedTemplateYear, setSelectedTemplateYear] = useState<number>(selectedGlobalYear);
    ...
    const handleCreateNewTemplate = () => {
        ...
        setFormName(`Szablon ${selectedTemplateYear}`);
        setFormSeparator(';');
        setFormYear(selectedTemplateYear);
    ...
    const handleSubmitTemplate = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Przełącz widok na rok zapisanego szablonu, aby użytkownik go widział
            setSelectedTemplateYear(templateToSave.year);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex space-x-1 bg-slate-100 p-1 rounded-xl w-fit overflow-x-auto max-w-full">
                <button
                    onClick={() => { setActiveMainTab('CSV'); setIsProcessing(false); }}
                    className={`px-6 py-2.5 rounded-lg text-sm font-semibold flex items-center gap-2 transition-all whitespace-nowrap ${activeMainTab === 'CSV'
                ...
                <button
                    onClick={() => { setActiveMainTab('RATES'); setIsProcessing(false); }}
                    className={`px-6 py-2.5 rounded-lg text-sm font-semibold flex items-center gap-2 transition-all whitespace-nowrap ${activeMainTab === 'RATES'
                        ? 'bg-white text-emerald-700 shadow-sm'
                        : 'text-slate-500 hover:text-slate-700'
                        }`}
                >
                ...
                <button
                    onClick={() => { setActiveMainTab('CROPS'); setIsProcessing(false); }}
                    className={`px-6 py-2.5 rounded-lg text-sm font-semibold flex items-center gap-2 transition-all whitespace-nowrap ${activeMainTab === 'CROPS'
                        ? 'bg-white text-emerald-700 shadow-sm'
                        : 'text-slate-500 hover:text-slate-700'
                        }`}
                >
                ...
                <button
                    onClick={() => { setActiveMainTab('DICT'); setIsProcessing(false); }}
                    className={`px-6 py-2.5 rounded-lg text-sm font-semibold flex items-center gap-2 transition-all whitespace-nowrap ${activeMainTab === 'DICT'
                ...
                <button
                    onClick={() => { setActiveMainTab('KB'); setIsProcessing(false); }}
                    className={`px-6 py-2.5 rounded-lg text-sm font-semibold flex items-center gap-2 transition-all whitespace-nowrap ${activeMainTab === 'KB'
                ...
                <button
                    onClick={() => { setActiveMainTab('RULES'); setIsProcessing(false); }}
                    className={`px-6 py-2.5 rounded-lg text-sm font-semibold flex items-center gap-2 transition-all whitespace-nowrap ${activeMainTab === 'RULES'
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    <div className="lg:col-span-1 bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                        <div className="flex justify-between items-center mb-4">
                            <h3 className="font-bold text-slate-800">Szablony CSV</h3>
                            <select
                                value={selectedTemplateYear}
                                onChange={(e) => setSelectedTemplateYear(Number(e.target.value))}
                                className="text-sm border-slate-300 rounded-lg p-1.5 bg-slate-50 font-semibold text-slate-700 focus:ring-1 focus:ring-emerald-500"
                            >
                        ...
                        <div className="space-y-3">
                            {activeTemplates.filter(t => !t.year || t.year === selectedTemplateYear).length === 0 && (
                                <p className="text-sm text-slate-400 italic text-center py-4">Brak szablonów dla roku {selectedTemplateYear}</p>
                            )}
                            {activeTemplates
                                .filter(t => !t.year || t.year === selectedTemplateYear)
                                .map(tpl => (
                            ...
                            <button onClick={handleCreateNewTemplate} className={`w-full py-3 border-2 border-dashed rounded-lg font-medium flex items-center justify-center gap-2 transition-colors mt-4 ${isCreatingTemplate && !editingTemplate ? 'border-emerald-500 text-emerald-600 bg-emerald-50' : 'border-slate-300 text-slate-500 hover:border-emerald-500'}`}>
                                <Plus size={18} /> Dodaj Szablon ({selectedTemplateYear})
                            </button>
                        </div>
                    </div>
                            ...
                            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 animate-in fade-in slide-in-from-right-4 duration-300">
                                <div className="flex justify-between items-center mb-6">
                                    ...
                                    <div className="flex gap-2">
                                        {editingTemplate && <button onClick={() => { if (window.confirm('Usunąć?')) { onDeleteTemplate(editingTemplate.id); setEditingTemplate(null); setIsProcessing(false); } }} className="text-red-500 hover:bg-red-50 p-2 rounded-lg"><Trash2 size={18} /></button>}
                                        <button onClick={() => { setIsCreatingTemplate(false); setEditingTemplate(null); setIsProcessing(false); }} className="text-slate-400 hover:bg-slate-100 p-2 rounded-lg"><X size={24} /></button>
                                    </div>
                                </div>
                                <form onSubmit={handleSubmitTemplate}>
                                    ...
                                    <div className="mt-6 flex justify-end gap-3"><button type="button" onClick={() => { setIsCreatingTemplate(false); setEditingTemplate(null); setIsProcessing(false); }} className="px-4 py-2 text-slate-500">Anuluj</button><button type="submit" disabled={isProcessing} className="bg-emerald-600 text-white px-6 py-2 rounded-lg flex items-center gap-2">{isProcessing && <Loader2 size={16} className="animate-spin" />} Zapisz Szablon</button></div>
                                </form>
                            </div>
                        ) : (
                            ...
                            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 animate-in fade-in slide-in-from-right-4 duration-300">
                                <div className="flex justify-between items-center mb-6">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <button onClick={() => { setIsCreatingRate(false); setEditingRate(null); setIsProcessing(false); }} className="text-slate-400 hover:bg-slate-100 p-2 rounded-lg"><X size={24} /></button>
                                    </div>
                                </div>
                                ...
                                <form onSubmit={handleSaveRate} className="space-y-4">
                                    ...
                                    <div className="pt-4 flex justify-end gap-3">
                                        <button type="button" onClick={() => { setIsCreatingRate(false); setEditingRate(null); setIsProcessing(false); }} className="px-4 py-2 text-slate-500 hover:bg-slate-50 rounded-lg">Anuluj</button>
                            ...
                            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 animate-in fade-in slide-in-from-right-4 duration-300">
                                <div className="flex justify-between items-center mb-6">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <button onClick={() => { setIsCreatingCrop(false); setEditingCrop(null); setIsProcessing(false); }} className="text-slate-400 hover:bg-slate-100 p-2 rounded-lg"><X size={24} /></button>
                                    </div>
                                </div>
                                ...
                                <form onSubmit={handleSaveCrop} className="space-y-4">
                                    ...
                                    <div className="pt-4 flex justify-end gap-3">
                                        <button type="button" onClick={() => { setIsCreatingCrop(false); setEditingCrop(null); setIsProcessing(false); }} className="px-4 py-2 text-slate-500 hover:bg-slate-50 rounded-lg">Anuluj</button>
                                        <button type="submit" disabled={isProcessing} className="px-6 py-2 bg-emerald-600 text-white rounded-lg hover:bg-emerald-700 shadow-sm font-medium flex items-center gap-2">
            ...
            {activeMainTab === 'RAG' && (
                <div className="animate-in fade-in duration-500">
                    <SemanticExplorer />  {/* ← używane ale NIE zaimportowane! */}
                </div>
            )}
        </div>
    );
