...
import { Field, CropType, CsvTemplate, FieldHistoryEntry, CsvTemplateType, FieldCropPart, getCampaignStatus } from '../types';
            ...
            <div className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col md:flex-row justify-between items-end gap-4">
                ...
                <div className="flex gap-3">
                    <div className="flex items-center gap-2 bg-slate-100 p-1 rounded-lg">
                        <select value={selectedYear} onChange={(e) => setSelectedYear(Number(e.target.value))} className="bg-transparent border-none px-4 py-1 font-bold text-sm focus:outline-none">
                            {availableYears.map(y => <option key={y} value={y}>{y}</option>)}
                        </select>
                        <div className={`px-2 py-0.5 rounded text-[10px] font-black uppercase border ${getCampaignStatus(selectedYear).color}`}>
