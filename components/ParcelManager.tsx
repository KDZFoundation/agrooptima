                ...
                return (
                  <tr key={field.id} className={`transition-colors ${isEditing ? 'bg-amber-50' : 'hover:bg-slate-50'}`}>
                    ...
                    <td className="p-4 font-mono text-sm">
                        {isEditing ? <input value={editForm.registrationNumber} onChange={e => setEditForm({...editForm, registrationNumber: e.target.value})} className="border border-amber-300 rounded px-2 py-1 text-sm w-24" /> : field.registrationNumber || '-'}
                    </td>
