...
const Dashboard: React.FC<DashboardProps> = ({ farmData, recentDocuments = [], onViewAllDocuments, onManageFields, selectedYear, setSelectedYear, onAddDocument }) => {
  ...
  const { 
    ...
    const bwsRateObj = yearRates.find(r => r.category === 'DOPLATA' && r.name.toLowerCase().includes('podstawowe')) || { rate: 0 };
    const redRateObj = yearRates.find(r => r.category === 'DOPLATA' && r.name.toLowerCase().includes('redystrybucyjna')) || { rate: 0 };
    const uppRateObj = yearRates.find(r => r.category === 'DOPLATA' && r.name.toLowerCase().includes('uzupełniająca')) || { rate: 0 };
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      try {
        ...
        // OCR i ekstrakcja tekstu dla bazy RAG bez klasyfikacji semantycznej (oszczędność czasu i unikanie błędów)
        ...
        const newDoc: FarmerDocument = {
          ...
          category: 'WNIOSEK',
          campaignYear: selectedYear.toString(), // Przypisujemy do aktualnie wybranego roku w UI
